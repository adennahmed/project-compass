-- ════════════════════════════════════════════════════════════════════════
-- Kozai Community — Idempotent re-seed of posts, comments, reactions, resources
-- ════════════════════════════════════════════════════════════════════════
-- The previous fix migration (20260517010000) seeded members successfully
-- but its content block silently failed for some users (no posts /
-- resources appeared in the UI). This file re-runs the content seed in
-- isolation with:
--
--   • `on conflict do nothing` on every insert (safe to re-run)
--   • `raise notice` after each block so the Supabase SQL editor shows
--     row counts in the output panel
--   • Hard guards that raise loudly if seed members are missing
--
-- Run it AFTER 20260517010000 has been applied. If members don't exist
-- yet, this migration will tell you exactly which one is missing.
-- ════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if not exists (select 1 from public.profiles where id = v_mira) then
    raise exception 'Seed member mira (id=%) missing — run 20260517010000 first.', v_mira;
  end if;
  if not exists (select 1 from public.profiles where id = v_dev) then
    raise exception 'Seed member devon missing.';
  end if;
  if not exists (select 1 from public.profiles where id = v_jules) then
    raise exception 'Seed member jules missing.';
  end if;
  if not exists (select 1 from public.profiles where id = v_theo) then
    raise exception 'Seed member theo missing.';
  end if;
  if not exists (select 1 from public.profiles where id = v_sana) then
    raise exception 'Seed member sana missing.';
  end if;
  if not exists (select 1 from public.profiles where id = v_kai) then
    raise exception 'Seed member kai missing.';
  end if;
  raise notice 'Members verified ✓';
  ...
  if c_ann is null or c_gen is null or c_show is null or c_hire is null or c_ops is null or c_ask is null then
    raise exception 'One or more channels missing. Re-run 20260515000000_community_initial.sql.';
  end if;
  raise notice 'Channels resolved ✓';
  ...
  insert into public.posts (id, author_id, channel_id, type, title, body_md, pinned) values
    (p_ann1, v_mira, c_ann, 'announcement', 'Kozai Community is open',
     E'Welcome. This is a small, deliberately quiet room for people who build the software ops teams actually use.\n\nNo growth hacks here. Post a thread, ask a question, share something you shipped.', true),
    (p_ann2, v_dev,  c_ann, 'announcement', 'We''re hiring a founding designer',
     E'Toronto-based, hybrid, ridiculous amount of taste required. The full thing is on our jobs page, but the short version: you''ll be the person who decides what every Kozai product feels like.', false),
    (p_ann3, v_mira, c_ann, 'announcement', 'Office hours, every other Wednesday',
     E'Starting next week — 30-minute slots, anything operations-software-related. Book through your member dashboard.', false),
    (p_gen1, v_jules, c_gen, 'thread', 'Hello room',
     E'Long-time lurker on the marketing site, first-time poster. Mostly here to read but happy to answer questions about scaling ops from 5 to 50 people. That''s the part I''ve done a few times.', false),
    (p_show1, v_theo, c_show, 'thread', 'Shipped a new invoice editor today',
     E'After three weeks of fighting `contentEditable` I finally have something that doesn''t corrupt itself when you paste from Excel.\n\nKey trick: stop trying to make rich text behave. Treat the document as a JSON tree and render to DOM, never the other way around. Obvious in hindsight.', false),
    (p_show2, v_kai, c_show, 'thread', 'Killed our internal admin tool — here''s what replaced it',
     E'We had a Retool app with 47 pages. Nobody used 40 of them. Replaced with a Linear-style command bar that opens 7 specific actions. Usage went up 6x.\n\nLesson: most internal tools are too generous. Cut features until people complain.', false),
    (p_hire1, v_sana, c_hire, 'thread', 'Hiring: senior backend, ops-software experience',
     E'Remote-friendly (NA timezones). We build software for industrial distributors. If you''ve worked on anything where the customer has a forklift, we want to talk.\n\nDM me, no formal application.', false),
    (p_ops1, v_jules, c_ops, 'thread', 'The case for boring internal tools',
     E'Spent a decade now watching teams over-engineer admin panels. The pattern: someone reads a "we built a beautiful internal tool" post, gets jealous, spends 6 weeks rebuilding their CRUD pages in a custom design system.\n\nNobody''s job got better.\n\nThe best internal tool I''ve ever used was a Django admin with three custom filters. Took an afternoon.', false),
    (p_ops2, v_dev, c_ops, 'thread', 'Naming convention: what do you call your "ops" repo',
     E'Ours is just `tools/`. Heard `internal/`, `ops/`, `admin/`, `runbook/`. Curious what people land on. Bonus points for the worst one you''ve inherited.', false),
    (p_ask1, v_kai, c_ask, 'question', 'How do you handle multi-tenant data in admin tooling?',
     E'Specifically — when a staff member needs to "look as" a customer to debug something. We''re doing it with a header swap right now, feels wrong but not sure of the right pattern.', false),
    (p_ask2, v_theo, c_ask, 'question', 'Best on-call rotation for a 3-person team?',
     E'We''re too small for PagerDuty pricing to make sense but customers are starting to expect off-hours response. What''s the bare minimum that doesn''t suck?', false)
  on conflict (id) do nothing;

  get diagnostics n_inserted = row_count;
  raise notice 'Posts inserted (new rows): %', n_inserted;
  ...
  get diagnostics n_inserted = row_count;
  raise notice 'Comments inserted (new rows): %', n_inserted;
  ...
  get diagnostics n_inserted = row_count;
  raise notice 'Reactions inserted (new rows): %', n_inserted;
  ...
  get diagnostics n_inserted = row_count;
  raise notice 'Resources inserted (new rows): %', n_inserted;

  -- ─── Final totals ──────────────────────────────────────────────────
  raise notice '─────────────────────────────────────';
  raise notice 'TOTAL posts in DB:     %', (select count(*) from public.posts);
  raise notice 'TOTAL comments in DB:  %', (select count(*) from public.comments);
  raise notice 'TOTAL reactions in DB: %', (select count(*) from public.reactions);
  raise notice 'TOTAL resources in DB: %', (select count(*) from public.resources);
  raise notice 'TOTAL profiles in DB:  %', (select count(*) from public.profiles);
end $$;
