-- ════════════════════════════════════════════════════════════════════════
-- Kozai Community — Fix seed (clean re-seed of placeholder members + content)
-- ════════════════════════════════════════════════════════════════════════
-- The prior seed (20260517000000) ran but produced inconsistent results
-- (only some members appeared; posts/resources missing). This migration
-- wipes the placeholder seed cleanly and re-runs with a corrected shape.
--
-- It ALSO force-promotes the bootstrap admin email so the role is correct
-- even if the user signed up before the trigger was deployed (or the
-- trigger was bypassed). This fixes the "no New resource / New announcement
-- button" issue when signed in as the bootstrap admin.
--
-- Safe to re-run.
-- ════════════════════════════════════════════════════════════════════════
...
-- whether the trigger was active at signup time.
update public.profiles p
  set role = 'admin',
      community_rules_accepted_at = coalesce(p.community_rules_accepted_at, now())
  from auth.users u
  where p.id = u.id and u.email = 'adenah04@outlook.com';

-- ─── 3. SEED PLACEHOLDER MEMBERS ────────────────────────────────────────
do $$
declare
...
begin
  -- Insert seed auth users one-by-one (so a single failure can't take down
  -- the whole batch). The handle_new_user trigger fires for each and
  -- creates a corresponding profiles row.
  insert into auth.users (
    ...
    (v_dev,   '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'devon@placeholder.kozai', '', now(), now(), now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Devon Park"}'::jsonb,      false, '', '', '', ''),
  ...
  -- Enrich the trigger-created profiles. Force handle so it lines up with
  -- the email local-part regardless of any collision-disambiguation the
  -- trigger applied.
...
-- ─── 4. SEED POSTS / COMMENTS / REACTIONS / RESOURCES ──────────────────
do $$
declare
  v_mira  uuid := '11111111-1111-1111-1111-111111111101';
  v_dev   uuid := '11111111-1111-1111-1111-111111111102';
...
begin
  ...
  if c_ann is null or c_gen is null or c_show is null or c_hire is null or c_ops is null or c_ask is null then
    raise exception 'Channel seed missing — cannot seed posts.';
  end if;
