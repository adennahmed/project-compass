...
const commands = ["help", "about", "projects", "kozai", "method", "stack", "contact", "clear"];
  ...
  return (
    <section id="console" data-snap className="relative px-6 py-24 md:px-10 md:py-28">
      <div className="container-wide">
        <Reveal><div className="mb-10 grid grid-cols-1 gap-6 md:mb-14 md:grid-cols-12 md:items-end"><div className="md:col-span-4"><div className="label"><ScrambleText text="[ 05 — Interactive console ]" /></div></div><div className="md:col-span-8"><h2 className="display text-ink" style={{ fontSize: "clamp(2rem, 5vw, 3.6rem)" }}>Poke around.<span className="text-mute"> It talks back.</span></h2><p className="mt-5 max-w-[52ch] text-[15px] leading-[1.6] text-mute md:text-[16px]">Type a command—or tap one below—to inspect the projects, archive, method, and tools. Try <span className="text-signal">help</span>.</p></div></div></Reveal>
        <Reveal delay={120}>
        ...
        <Reveal delay={190}><div className="mx-auto mt-5 flex max-w-[820px] flex-wrap items-center gap-2"><span className="mr-1 font-mono text-[9px] uppercase tracking-[0.2em] text-mute">try:</span>{["help", "projects", "kozai", "method", "stack", "contact"].map((command) => <button key={command} type="button" onClick={() => submit(command)} className="border border-hairline/20 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.15em] text-mute transition-colors hover:border-signal hover:text-signal">{command}</button>)}</div></Reveal>
      </div>
    </section>
  );
};
